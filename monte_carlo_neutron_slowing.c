...
#define SOURCE_SPEED_RATIO 50.0
...
typedef struct {
    ...
    // Speed bins for tallies
    int num_bins;
    double *speed_bins;

    // Tallies
    long long *scattering_tally;
    long long *absorption_tally;

    // For flux calculation
    double *scatter_sum;
    double *scatter_sum_sq;

    // Statistics
    long long total_histories;
    long long total_scatterings;
    long long total_absorptions;

    // RNG state
    uint64_t rng_state;
} Simulation;

// Target velocity sampling structure
typedef struct {
    double V;
    double mu;
} TargetSample;
...
void init_simulation(Simulation* sim, double A, double K, double T, int num_bins);
void free_simulation(Simulation* sim);
void run_simulation(Simulation* sim, long num_histories);
void run_history(Simulation* sim);
void calculate_flux(Simulation* sim, double* y, double* flux, double* flux_err, int* count);
...
TargetSample sample_target_velocity(Simulation* sim, double v);
...
void free_simulation(Simulation* sim) {
    free(sim->speed_bins);
    free(sim->scattering_tally);
    free(sim->absorption_tally);
    free(sim->scatter_sum);
    free(sim->scatter_sum_sq);

    sim->speed_bins = NULL;
    sim->scattering_tally = NULL;
    sim->absorption_tally = NULL;
    sim->scatter_sum = NULL;
    sim->scatter_sum_sq = NULL;
}

// Calculate flux with error for metrics extraction
typedef struct {
    double peak_y;
    double peak_flux;
    double tail_slope;
...
SpectrumMetrics extract_metrics(Simulation* sim) {
    double *y = malloc(sim->num_bins * sizeof(double));
    double *flux = malloc(sim->num_bins * sizeof(double));
    double *err = malloc(sim->num_bins * sizeof(double));

    if (!y || !flux || !err) {
        SpectrumMetrics m = {0.0, 0.0, NAN};
        free(y);
        free(flux);
        free(err);
        return m;
    ...
    int valid_points = 0;
    calculate_flux(sim, y, flux, err, &valid_points);
    (void)valid_points;
    ...
    for (int i = 0; i < sim->num_bins; i++) {
        if (y[i] > 0.5 && y[i] < 2.5 && flux[i] > peak_val) {
            peak_val = flux[i];
            peak_idx = i;
        }
    }
    ...
    for (int i = 0; i < sim->num_bins; i++) {
        if (y[i] >= 3.0 && y[i] <= 7.0 && flux[i] > 0.0) {
            double lx = log(y[i]);
            double ly = log(flux[i]);
            sumx += lx;
            sumy += ly;
            sumxx += lx * lx;
            sumxy += lx * ly;
    ...
    if (n > 2) {
        m.tail_slope = (n * sumxy - sumx * sumy) / (n * sumxx - sumx * sumx);
    } else {
        m.tail_slope = NAN;
    }
...
void init_simulation(Simulation* sim, double A, double K, double T, int num_bins) {
    ...
    // Calculate thermal speed (speed at the peak of the flux)
    ...
    // Allocate memory for speed bins and tallies
    sim->speed_bins = calloc(num_bins + 1, sizeof(double));
    sim->scattering_tally = calloc(num_bins, sizeof(long long));
    sim->absorption_tally = calloc(num_bins, sizeof(long long));
    sim->scatter_sum = calloc(num_bins, sizeof(double));
    sim->scatter_sum_sq = calloc(num_bins, sizeof(double));

    // Initialize speed bins
    double max_speed = MAX_SPEED_RATIO * sim->v_T;
    for (int i = 0; i <= num_bins; i++) {
        sim->speed_bins[i] = i * max_speed / num_bins;
    }
    ...
    // Initialize RNG
    sim->rng_state = 2005;  // fixed seed for reproducibility
}
...
TargetSample sample_target_velocity(Simulation* sim, double v) {
    ...
    while (!accepted) {
        ...
        if (eta < P2 / (P2 + P3)) {
            ...
            double f_max = C2 * (1.0 / alpha_moderator) * exp(-1.0); // true maximum
            ...
            double f_max = C3 * pow(3.0 / (2.0 * alpha_moderator), 1.5) * exp(-1.5); // true maximum
...
void elastic_collision(Simulation* sim, double* v, double V, double mu) {
    ...
    if (cos_theta_cm > 1.0) cos_theta_cm = 1.0;
    if (cos_theta_cm < -1.0) cos_theta_cm = -1.0;
...
void run_history(Simulation* sim) {
    ...
    int hist_scatter[NUM_INTERVALS] = {0}; 
    int hist_absorb[NUM_INTERVALS] = {0};
    ...
    while (collision_count < MAX_COLLISIONS) {
        ...
        double max_v = MAX_SPEED_RATIO * sim->v_T;
        int bin_idx = (int)(v / max_v * sim->num_bins);

        // Safety clamp (The "FP overshoot" protection)
        if (bin_idx < 0) bin_idx = 0;
        if (bin_idx >= sim->num_bins) bin_idx = sim->num_bins - 1;
        ...
        if (random_uniform(sim) < absorption_prob) {
            ...
            // Scattering event
            hist_scatter[bin_idx]++;
            sim->total_scatterings++;

            // Sample target velocity using paper's rejection method
            TargetSample Target = sample_target_velocity(sim, v);

            // Use 3D collision physics
            elastic_collision(sim, &v, Target.V, Target.mu);
    ...
    for (int i = 0; i < sim->num_bins; i++) {
        sim->scattering_tally[i] += hist_scatter[i];
        sim->absorption_tally[i] += hist_absorb[i];

        sim->scatter_sum[i] += (double)hist_scatter[i];
        sim->scatter_sum_sq[i] += (double)hist_scatter[i] * (double)hist_scatter[i];
    }
...
void run_simulation(Simulation* sim, long num_histories) {
    printf("Running %ld neutron histories...\n", num_histories);
    printf("Parameters: A=%.2f, K=%.2f, T=%.0f K\n", sim->A, sim->K, sim->T);
    printf("Thermal speed: %.2e m/s\n", sim->v_T);

    clock_t start = clock();

    for (long i = 0; i < num_histories; i++) {
        if ((i + 1) % 10000 == 0) {
            printf("Completed %ld histories\n", i + 1);
        }
        run_history(sim);
    }
    ...
    printf("Total scatterings: %lld\n", sim->total_scatterings);
    printf("Total absorptions: %lld\n", sim->total_absorptions);
    printf("Average collisions per history: %.2f\n",
           (double)(sim->total_scatterings + sim->total_absorptions) / (double)sim->total_histories);
}

// Calculate flux from tallies
void calculate_flux(Simulation* sim, double* y, double* flux, double* flux_err, int* count) {
    double H = (double)sim->total_histories;
    ...
    for (int i = 0; i < sim->num_bins; i++) {
        ...
        if (sim->total_histories > 0 && bin_width > 0.0) {
            double mean = sim->scatter_sum[i] / H;
            double var = 0.0;

            if (sim->total_histories > 1) {
                var = (sim->scatter_sum_sq[i] - H * mean * mean) / (H - 1.0);
                if (var < 0.0) var = 0.0;
            }

            double sigma_es = calculate_effective_sigma_s(sim, bin_center);
            double scale = sigma_es * bin_width;

            if (scale > 0.0) {
                flux[i] = mean / scale;
                flux_err[i] = sqrt(var / H) / scale;
                valid_points++;
            } else {
                flux[i] = 0.0;
                flux_err[i] = 0.0;
    ...
    // but exclude region for (y > 8.0)
    double max_flux = 0.0;
    for (int i = 0; i < sim->num_bins; i++) {
        if (y[i] > 0.5 && y[i] < 2.5 && y[i] < 8.0 && flux[i] > max_flux) {
            max_flux = flux[i];
        }
    }
...
void print_results(Simulation* sim) {
    ...
    double flux_err[NUM_INTERVALS];
    int valid_points = 0;

    calculate_flux(sim, y, flux, flux_err, &valid_points);

    printf("\nRESULTS\n");
    printf("Valid data points: %d\n", valid_points);
    printf("y\tFlux\t\tErr\t\tMB Flux\n");
    printf("---\t-----\t\t---\t\t--------\n");

    int step = NUM_INTERVALS / 16;
    if (step < 1) step = 1;

    for (int i = 0; i < NUM_INTERVALS; i += step) {
        if (flux[i] > 0) {
            double mb_flux = maxwell_flux(y[i]);
            printf("%.3f\t%.6f\t%.6f\t%.6f\n", y[i], flux[i], flux_err[i], mb_flux);
        }
    }
}
...
void save_results(Simulation* sim, const char* filename) {
    ...
    fprintf(file, "y,flux,flux_err,mb_flux\n");
...
void test_carbon_energy_loss() {
    ...
    Simulation sim;
    init_simulation(&sim, 12.0, 0.03125, 293.0, NUM_INTERVALS);
...
void run_convergence_study(double A, double K, double T) {
    long hist_list[] = {10000, 50000, 100000, 500000, 1000000};
    int n_cases = sizeof(hist_list) / sizeof(hist_list[0]);

    FILE *fp = fopen("convergence_histories.csv", "w");
    if (!fp) {
        printf("Error opening file convergence_histories.csv\n");
        return;
    }

    fprintf(fp, "histories,peak_y,peak_flux,tail_slope\n");

    for (int i = 0; i < n_cases; i++) {
        Simulation sim;
        init_simulation(&sim, A, K, T, NUM_INTERVALS);

        run_simulation(&sim, hist_list[i]);
        SpectrumMetrics m = extract_metrics(&sim);

        fprintf(fp, "%ld,%.8f,%.8f,%.8f\n",
                hist_list[i], m.peak_y, m.peak_flux, m.tail_slope);
...
int main() {
    ...
    Simulation sim;
    init_simulation(&sim, 1.0, 0.18, 293.0, NUM_INTERVALS);
    run_simulation(&sim, NUM_HISTORIES);
    print_results(&sim);
    save_results(&sim, "neutron_flux_hydrogen.csv");
    free_simulation(&sim);

    // Higher absorption case (A=1), K=0.36
    printf("\nCase 2: High absorption (A=1.0, K=0.36)\n");
    Simulation sim_highK;
    init_simulation(&sim_highK, 1.0, 0.36, 293.0, NUM_INTERVALS);
    run_simulation(&sim_highK, NUM_HISTORIES);
    print_results(&sim_highK);
    save_results(&sim_highK, "neutron_flux_highK.csv");
    free_simulation(&sim_highK);

    // Carbon moderator (A=12)
    printf("\nCase 3: Carbon moderator (A=12.0, K=2^-5)\n");
    test_carbon_energy_loss();
    Simulation sim_carbon;
    init_simulation(&sim_carbon, 12.0, 0.03125, 293.0, NUM_INTERVALS);
    run_simulation(&sim_carbon, NUM_HISTORIES);
    print_results(&sim_carbon);
    save_results(&sim_carbon, "neutron_flux_carbon.csv");
    free_simulation(&sim_carbon);

    // Helium moderator (A=2)
    printf("\nCase 4: Helium moderator (A=2.0, K=2^-5)\n");
    Simulation sim_helium;
    init_simulation(&sim_helium, 2.0, 0.03125, 293.0, NUM_INTERVALS);
    run_simulation(&sim_helium, NUM_HISTORIES);
    print_results(&sim_helium);
    save_results(&sim_helium, "neutron_flux_helium.csv");
    free_simulation(&sim_helium);

    // Silicon moderator (A=16)
    printf("\nCase 5: Silicon moderator (A=16.0, K=2^-5)\n");
    Simulation sim_silicon;
    init_simulation(&sim_silicon, 16.0, 0.03125, 293.0, NUM_INTERVALS);
    run_simulation(&sim_silicon, NUM_HISTORIES);
    print_results(&sim_silicon);
    save_results(&sim_silicon, "neutron_flux_silicon.csv");
    free_simulation(&sim_silicon);

    // Manganese moderator (A=25)
    printf("\nCase 6: Manganese moderator (A=25.0, K=2^-5)\n");
    Simulation sim_manganese;
    init_simulation(&sim_manganese, 25.0, 0.03125, 293.0, NUM_INTERVALS);
    run_simulation(&sim_manganese, NUM_HISTORIES);
    print_results(&sim_manganese);
    save_results(&sim_manganese, "neutron_flux_manganese.csv");
    free_simulation(&sim_manganese);
    ...
    run_convergence_study(1.0, 0.18, 293.0);

    printf("  - neutron_flux_hydrogen.csv\n");
    printf("  - neutron_flux_highK.csv\n");
    printf("  - neutron_flux_carbon.csv\n");
    printf("  - neutron_flux_helium.csv\n");
    printf("  - neutron_flux_silicon.csv\n");
    printf("  - neutron_flux_manganese.csv\n");
    printf("  - convergence_histories.csv\n");
